...
public class StopWordFilter extends BaseRichBolt {
	...
	public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
		...
		this.stopWordList = new HashSet(Arrays.asList(stopwords.split("\\|")));
	}

	@Override
	public void execute(Tuple input) {
		String words = (String) input.getValue(1);
		List<String> wordsSplit = Arrays.asList(words.split("#"));
		for(String word: wordsSplit)
		{
			...
			if (!stopWordList.contains(lowerCaseWord) && !lowerCaseWord.startsWith("http")
				&& lowerCaseWord.matches(".*[a-zA-Z]+.*")) {
				_collector.emit(new Values(input.getValue(0), lowerCaseWord));	
			}
		}
	}
